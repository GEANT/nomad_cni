...
class nomad_cni (
  ...
  Variant[Integer, String] $firewall_rule_order            = 050,
  ...
  # == Firewall setting
  #
  if regsubst($firewall_rule_order, '^0+', '') =~ Integer {
    fail('firewall_rule_order must be an integer or digits that starts with zero(es) number')
  }
